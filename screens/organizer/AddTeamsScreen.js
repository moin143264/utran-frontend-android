...
const AddTeamsScreen = ({ route, navigation }) => {
    ...
    const handleAddTeam = () => {
        if (teamList.length >= competition.maxTeams) {
            Alert.alert('Error', 'Maximum number of teams reached');
            return;
        }
    ...
    const handleSubmit = async (formData) => {
        try {
            ...
            Alert.alert('Success', `Team ${selectedTeam ? 'updated' : 'added'} successfully`);
        } catch (error) {
            Alert.alert('Error', error.message || `Failed to ${selectedTeam ? 'update' : 'add'} team`);
        } finally {
            setLoading(false);
        }
    };
