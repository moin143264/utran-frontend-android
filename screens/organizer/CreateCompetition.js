...
const CreateCompetition = ({ navigation }) => {
  ...
  const validateForm = () => {
    ...
    if (isNaN(parseInt(formData.maxTeams))) {
      Alert.alert("Error", "Please enter a valid number for maximum teams");
      return false;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Format prizes - try to parse prize text into an array of position/prize objects
      let formattedPrizes = [];
      try {
        const prizeLines = formData.prizes
          .split("\n")
          .map((line) => line.trim())
          .filter((line) => line.length > 0);

        formattedPrizes = prizeLines.map((line) => {
          // Try to match "position: prize" format
          ...
          // Default to just using the line as prize with numbered position
          return {
            position: `Position ${formattedPrizes.length + 1}`,
            prize: line,
          };
        });
      } catch (error) {
        console.error("Error formatting prizes:", error);
        formattedPrizes = [{ position: "1st", prize: formData.prizes }];
      }
      ...
      console.log("Creating competition with data:", requestBody);

      // Call API to create competition
      const response = await competitions.create(requestBody);
