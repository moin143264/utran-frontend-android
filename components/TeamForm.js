...
const TeamForm = ({ 
    ...
    const handleCaptainLookup = async () => {
        if (!captainEmail) {
            Alert.alert('Error', 'Please enter a captain email');
            return;
        }
        ...
        if (user) {
            ...
            if (user.role === 'admin') {
                Alert.alert('Error', 'Admin users cannot be added as team captain');
                setCaptainData(null);
                return;
            }
            
            setCaptainData(user);
            Alert.alert('Success', `Found captain: ${user.name}`);
        } else {
            setCaptainData(null);
    ...
    const handlePlayerLookup = async (index) => {
        ...
        if (captainData && captainData.email.toLowerCase() === playerEmail.toLowerCase()) {
            Alert.alert('Error', 'This user is already added as the team captain');
            return;
        }
        ...
        if (isDuplicate) {
            Alert.alert('Error', 'This player is already added to the team');
            return;
        }
        ...
        if (user) {
            // Check if user has admin role
            if (user.role === 'admin') {
                Alert.alert('Error', 'Admin users cannot be added as team players');
            ...
            setPlayers(updatedPlayers);
            Alert.alert('Success', `Found player: ${user.name}`);
        } else {
            const updatedPlayers = [...players];
            updatedPlayers[index].userData = null;
            setPlayers(updatedPlayers);
